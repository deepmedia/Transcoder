...
import static com.otaliastudios.transcoder.Transcoder.SUCCESS_NOT_NEEDED;
import static com.otaliastudios.transcoder.Transcoder.SUCCESS_TRANSCODED;
...
public class DefaultEngine implements Engine {
    ...
    private static final Logger LOG = new Logger("Engine");

    private static final long SLEEP_TO_WAIT_TRACK_TRANSCODERS = 10;
    private static final long PROGRESS_INTERVAL_STEPS = 10;

    private DataSink mDataSink;
    private final TrackMap<List<DataSource>> mDataSources = new TrackMap<>();
    private final TrackMap<ArrayList<TrackTranscoder>> mTranscoders = new TrackMap<>(new ArrayList<TrackTranscoder>(), new ArrayList<TrackTranscoder>());
    private final TrackMap<ArrayList<TimeInterpolator>> mInterpolators = new TrackMap<>(new ArrayList<TimeInterpolator>(), new ArrayList<TimeInterpolator>());
    private final TrackMap<Integer> mCurrentStep = new TrackMap<>(0, 0);
    private final TrackMap<TrackStatus> mStatuses = new TrackMap<>();
    private final TrackMap<MediaFormat> mOutputFormats = new TrackMap<>();
    ...
    @Override
    public void transcode(@NonNull TranscoderOptions options) throws Exception {
        Dispatcher dispatcher = new Dispatcher(options.getListenerHandler(), options.getListener());
        try {
            transcode(options, dispatcher);
        } catch (Throwable e) {
    ...
    /**
     * Does transcoding, dispatching progress and success. Failure and cancellation are
     * communicated through exceptions.
     */
    private void transcode(
        ...
        if (!options.getValidator().validate(videoStatus, audioStatus) && !ignoreValidatorResult) {
            ...
            dispatcher.onTranscodeCompleted(SUCCESS_NOT_NEEDED);
            return;
        }
        ...
        try {
            ...
            while (!(audioCompleted && videoCompleted)) {
                ...
                if (++loopCount % PROGRESS_INTERVAL_STEPS == 0) {
                    ...
                    dispatcher.onTranscodeProgress((videoProgress + audioProgress) / activeTracks);
                }
                if (!stepped) {
        ...
        dispatcher.onTranscodeCompleted(SUCCESS_TRANSCODED);
    }
}
